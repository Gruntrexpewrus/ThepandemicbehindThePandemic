...
class LaRepubblicaParser:
    ...
    def get_updated_dataset(self):
        ...
        for index, count in zip(self.dataset.index, range(num_of_articles)):
            ...
            try:
                ...
                page_content = response.data.decode(self.decode_format, errors='ignore')
    ...
    def _extract_article_body(self, page_content):
        ...
        body = soup.find('span', {'id':'article-body'})
        ...
        try:
            for div in body.find_all("section", {'class':'inline-video'}): 
                div.decompose()
        except:
            pass
