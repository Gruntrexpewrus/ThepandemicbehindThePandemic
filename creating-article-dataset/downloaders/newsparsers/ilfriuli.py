...
class IlFriuliParser:
    ...
    def __extract_date(self, raw_string):
    ...
    def get_updated_dataset(self):
        ...
        for index, count in zip(self.dataset.index, range(num_of_articles)):
            ...
            try:
                ...
                updated_article_content, date = self._extract_article_body(page_content)
                
                print("\tOverwriting dataset...")
                
                if date != None:
                    self.dataset['content'].iloc[index] = updated_article_content
                    self.dataset['date'].iloc[index] = date
    ...
    def _extract_article_body(self, page_content):
        ...
        try:
            article_date = self.__extract_date(soup.find('div', {'class':'article__date'}).getText())
        except:
            article_date = None
            pass
        ...
        '''
        #print(updated_content)
        return updated_content, article_date
