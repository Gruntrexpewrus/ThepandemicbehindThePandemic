...
html_paths = glob.glob("/home/marco/workspace/git/StatLearnTeam/web_pages_index/new_pages/aostanews*.html")
for path in html_paths: # From last page to most recent
    ...
    article_section = soup.findAll('div', attrs={"class":"content-row"})
    
    for article in article_section:
        try:
            title = article.find('h4', {'class':'entry-title'}).getText()
            content =  ''#  
            
            date = extract_date(article.find('time', {'class':'entry-date'}).getText())
            
            author = 'AostaNews'
            
            url = article.find('h4', {'class':'entry-title'}).find('a')['href']
            website = 'www.gazzettamatin.com'
...
articles_df.to_csv('/home/marco/workspace/git/StatLearnTeam/dataset/aostanews.csv', 
                   sep=';',
                   na_rep='NULL',
                   columns=cols,
                   escapechar="")
