...
html_paths = glob.glob("/home/marco/workspace/git/StatLearnTeam/web_pages_index/new_pages/nuova_sardegna*.html")
for path in html_paths: # From last page to most recent
    ...
    article_section = soup.findAll('div', attrs={"class":"block_item"})
    
    for article in article_section:
        try:
            title = article.find('h2', {'class':'entry_title'}).getText()
            content =  ''#
            
            date =  extract_date(article.find('div', {'class':'entry_content'}).find('time').getText())
            
            author = 'La Nuova Sardegna'
            
            url = article.find('h2', {'class':'entry_title'}).find('a')['href']
            website = 'www.lanuovasardegna.it'
...
articles_df.to_csv('/home/marco/workspace/git/StatLearnTeam/dataset/nuova_sardegna.csv', 
                   sep=';',
                   na_rep='NULL',
                   columns=cols,
                   escapechar="")
