...
html_paths = glob.glob("/home/marco/workspace/git/StatLearnTeam/web_pages_index/new_pages/cnaemiliaromagna*.html")
for path in html_paths: # From last page to most recent
    ...
    article_section = soup.findAll('article', {'class':'post'})
    
    for article in article_section:
        try:
            title = article.find('h3', {'class':'card-title'}).find('a').getText()
            content =  ''#  
            
            date = extract_date(article.find('div', {'class':'date'}).getText())
            
            author = 'CNA Emilia Romagna'
            
            url = article.find('h3', {'class':'card-title'}).find('a')['href']
            
            website = 'www.cnaemiliaromagna.it'
...
articles_df.to_csv('/home/marco/workspace/git/StatLearnTeam/dataset/cnaemiliaromagna.csv', 
                   sep=';',
                   na_rep='NULL',
                   columns=cols,
                   escapechar="")
