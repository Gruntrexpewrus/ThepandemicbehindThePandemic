...
def extract_date(raw_string):
    ...
    raw_string = raw_string.replace('Tutte le notizie del ', '')
...
html_paths = glob.glob("/home/marco/workspace/git/StatLearnTeam/web_pages_index/new_pages/basilicata*.html")
for path in html_paths: # From last page to most recent
    ...
    article_section = soup.findAll('article', attrs={"class":"news-item"})
    date = ''
    
    try:
        date = extract_date(soup.find('h3', {'class':'archive-title'}).getText())
    except Exception as e:
        print('Error ', e, ' on page ', path)
        pass
    
    for article in article_section:
        try:
            title = article.find('h1').find('a').getText()
            content =  ''#  
            
            
            author = 'BasilicataNews24'
            
            url =  article.find('h1').find('a')['href']
            website = 'www.basilicata24.it'
...
articles_df.to_csv('/home/marco/workspace/git/StatLearnTeam/dataset/basilicatanews24.csv', 
                   sep=';',
                   na_rep='NULL',
                   columns=cols,
                   escapechar="")
